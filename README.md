...
- Убрана повторная инициализация платежа (`Init`) при уже созданной ссылке оплаты.
- Добавлен fallback при ошибке `order_id already exists` (`ErrorCode=8`) при пустой локальной ссылке.
- Добавлено сохранение ссылки оплаты в примечание к заказу.
- Улучшена обработка статусов `AUTH_FAIL` и `REJECTED` для WooCommerce (`failed` + очистка устаревших мета платежа).
